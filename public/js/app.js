...
new Vue({
	...
	methods: {
		...
		getReview: function getReview() {
			...
			axios.get('/review/api').then(function (response) {
				return _this3.reviews = response.data;
			}).catch(function (error) {
				return console.log(error);
			});
		},
		...
		deleteReview: function deleteReview(review) {
			...
			axios.delete('/review/' + review.id).then(function (response) {
				return _this4.getReview();
			});
